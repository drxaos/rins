...
public class Ssr implements NanoExecution {
    public static final Ssr IMPL_I = new Ssr(Indirect.IMPL);
    public static final Ssr IMPL_D = new Ssr(Direct.IMPL);
    public static final Ssr IMPL_F = new Ssr(Flags.IMPL);
    public static final Ssr IMPL_A = new Ssr(VarA.IMPL);

    private final Addressing target;

    public Ssr(Addressing target) {
        this.target = target;
    }
    ...
    public void execute(NanoCpu nanoCpu, int pc, byte ins, NanoMemory mem, NanoIo io) {
        ...
        final byte result = (byte) (data >> count);
